...
const findNodePathsWithLocation = (
	ast: t.File,
	location: ExpressionLocation,
) => {
...
abstract class SingleLocationInstruction implements Instruction {
	...
	async process(data, cache) {
		...
		const nodePaths = findNodePathsWithLocation(ast, this.location).filter(
			(nodePath) => this.filterNodePath(nodePath),
		);
...
const evaluationDidSomethingGoodOrCrashed = (
	evaluation: MutationEvaluation,
) => {
...
class DeleteStatementInstruction implements Instruction {
	...
	recalculateMutationResults() {
		...
		while (s < stack.length) {
			...
			const key = locationToKeyIncludingEnd(
				statement.filePath,
				statement.location!,
			);
		...
		for (const statement of statements) {
			lineWidth += Math.abs(
				statement.location.end.line - statement.location.start.line,
			);
		...
		for (const statementBlock of this.statementBlocks.unsortedIterator()) {
			if (statementBlock.statements[0].retries < this.maxRetries) {
				if (
					...
					statementBlock.statements.length >
						largestStatementBlock.statements.length
				) {
 ...
 */
export const matchAndFlattenCategoryData = <T>(
	match: T,
...
class ReplaceNumberFactory
	implements InstructionFactory<ReplaceNumberInstruction>
{
...
class ReplaceStringFactory
	implements InstructionFactory<ReplaceStringInstruction>
{
	enter() {}
	...
	*createInstructions(nodePath, filePath, derivedFromPassingTest) {
		...
		if (nodePath.isStringLiteral() && node.loc) {
			...
			const values = node[STRINGS]!.filter(
	...
	*createInstructions(path: NodePath, filePath, derivedFromPassingTest) {
		...
		if (couldBeVariableNameReplaceable(path) && node.loc) {
			...
			if (filteredNames.length > 0) {
				yield createInstructionHolder(
					new ReplaceIdentifierInstruction(
						{ filePath, ...node.loc },
						filteredNames,
					),
					derivedFromPassingTest,
...
class AssignmentFactory implements InstructionFactory<AssignmentInstruction> {
	...
	*createInstructions(path, filePath, derivedFromPassingTest) {
		...
		if (path.isAssignmentExpression() && node.loc) {
			const operators = matchAndFlattenCategoryData(
				node.operator,
				this.operations,
			);
...
async function identifyUnknownInstruction(
	...
	for (const filePath of filePaths) {
		...
		const enter = (path: NodePath) => {
			...
			nodeCounts.push(
				path.isBlockStatement() || path.isExpressionStatement() ? 0 : 1,
			);
			for (const instructionFactory of instructionFactories) {
				instructionFactory.onInitialEnter(
					path,
					filePath,
					derivedFromPassingTest,
				);
			...
			if (statementStack.length > 0) {
				...
				for (const factory of instructionFactories) {
					statement.instructionHolders.push(
						...factory.createPreBlockInstructions(
							path,
							filePath,
							derivedFromPassingTest,
						),
...
export const compareMutationEvaluationsWithLesserProperties = (
	...
	if (!r1.crashed && !r2.crashed) {
		...
		const lineScoreNulls =
			stackEval1.lineScoreNulls - stackEval2.lineScoreNulls;
		if (lineScoreNulls !== 0) {
			return lineScoreNulls;
		}

		const columnScoreNulls =
			stackEval1.columnScoreNulls - stackEval2.columnScoreNulls;
...
export const compareMutationEvaluations = (
	...
	if (
		...
		const isDeleteStatement =
			(r2.type === DELETE_STATEMENT ? 1 : -1) -
			(r1.type === DELETE_STATEMENT ? 1 : -1);
...
export const evaluateModifiedTestResult = (
	...
	const samePassFailResult =
		originalResult.data.passed === newResult.data.passed;
	...
	const errorChanged: boolean | null = (() => {
		...
		return (
			(newResult.data as any).stack !==
			(originalResult.data as FailingTestData).stack
		);
	})();
...
export type MutationEvaluation =
	| CrashedMutationEvaluation
	| NormalMutationEvaluation;
...
const evaluateNewMutation = (
	...
	let testsWorsened = 0;
	let testsImproved = 0;
	const stackEvaluation: MutationStackEvaluation =
		createMutationStackEvaluation();
	let errorsChanged = 0;
	...
	for (const [key, newResult] of newResults.testResults) {
		...
		if (testEvaluation.endResultChange === EndResult.BETTER) {
			...
			(testEvaluation.stackLineScore === 0 ||
				testEvaluation.stackColumnScore === null) &&
			(testEvaluation.stackColumnScore === 0 ||
				testEvaluation.stackColumnScore === null)
		) {
			errorsChanged++;
			overallPositiveEffect++;
...
const compareMutationEvaluationsWithLargeMutationCountsFirst = (
	...
	const partial =
		(b.evaluation.partial ? 1 : -1) - (a.evaluation.partial ? 1 : -1);
	if (partial !== 0) {
		return partial;
	}

	const atomicMutation =
		(a.evaluation.atomicMutation ? 1 : -1) -
		(b.evaluation.atomicMutation ? 1 : -1);
	if (atomicMutation !== 0) {
	...
	const crashed =
		(b.evaluation.crashed ? 1 : -1) - (a.evaluation.crashed ? 1 : -1);
	if (crashed !== 0) {
		return crashed;
	}

	return compareMutationEvaluationsWithLesserProperties(
		a.evaluation,
		b.evaluation,
	);
};
...
export const compareLocationEvaluations = (
	...
	do {
		...
		aI < aSingleMutationsOnly.length &&
		bI < bSingleMutationsOnly.length
	);
...
export const mutationEvalatuationMapToFaults = (
	...
	const locationEvaluationsList: LocationEvaluation[] = [
		...locationEvaluations.values(),
	];
...
export const createDefaultIsFinishedFn = ({
	...
	const isFinishedFn: IsFinishedFunction = (
		...
		if (
			finishOnPassDerviedNonFunctionInstructions &&
			data.derivedFromPassingTest
		) {
			return true;
		}
		...
		if (data.mutationEvaluations.length > 0) {
			...
			const hasAtomicMutations = data.mutationEvaluations.some(
				(e) => e.atomicMutation,
			);
			...
			const mostSpecificMutationsOnlyContainCrashes =
				mostSpecificMutations.filter((evaluation) => evaluation.crashed)
					.length === mostSpecificMutations.length;
			if (!mostSpecificMutationsOnlyContainCrashes) {
				const containsUsefulMutations = data.mutationEvaluations.some(
					(evaluation) => {
						const improved =
							!evaluation.crashed &&
							(evaluation.testsImproved > 0 ||
						...
						const nothingChangedInNonDeleteStatement =
							...
							evaluation.testsImproved === 0 &&
							evaluation.testsWorsened === 0 &&
							nothingChangedMutationStackEvaluation(
...
export const isLocationWithinBounds = (
	...
	const onStartLineBound =
		...
		(loc.start.line !== statement.end.line ||
			loc.start.column <= statement.end.column);
			...
			if (
				...
				if (mostRelevantStatement === null) {
					...
					const columnDistance = Math.abs(
						statement.start.column - loc.start.column,
					);
					...
					if (
						...
						const lineWidth = Math.abs(
							statement.end.line - statement.start.line,
						);
						const columnWidth = Math.abs(
							statement.end.column - statement.start.column,
						);
						...
						const oColumnWidth = Math.abs(
							mostRelevantStatement.end.column -
								mostRelevantStatement.start.column,
						);
...
const resetMutationsInInstruction = async (instruction: InstructionHolder) => {
	...
	await Promise.all(
		Object.keys(previousMutationResults.locations).map((filePath) =>
			resetFile(filePath),
		),
	);
};
...
export const createPlugin = ({
	...
	const resolvedIgnoreGlob = (
		Array.isArray(ignoreGlob) ? ignoreGlob : [ignoreGlob]
	...
	const analyzeEvaluation = async (
		...
		if (previousInstruction !== null) {
			const previousMutationResults =
				previousInstruction.instruction.mutationResults;

			// Revert all mutated files
			await Promise.all(
				Object.keys(previousMutationResults.locations).map(resetFile),
			);

			if (
				!(previousInstruction.instruction instanceof DeleteStatementInstruction)
			) {
	...
	return {
		on: {
			...
			allFilesFinished: async (tester: TesterResults) => {
				...
				if (firstRun) {
						...
						for (const instructionHolder of popped.instructionHolders) {
							for (const [filePath, locations] of Object.entries(
								...
								for (const location of locations) {
									...
									allLocations.set(instructionKey, {
										...location.location,
										filePath,
									});
								}
					...
					if (previousRunWasPartial && !mutationEvaluation.crashed) {
						const testsToBeRerun = [
							...firstTesterResults.testResults.values(),
					...
					if (
						mutationEvaluation.testsImproved ===
							[...firstTesterResults.testResults.values()].filter(
								(a) => !a.data.passed,
							).length &&
						...
						const newSolutionDir = resolve(
							solutionsDir,
							(solutionCounter++).toString(),
						);
						...
						for (const filePath of Object.keys(locationObj)) {
							const code = await readFile(filePath, "utf8");
							await writeFile(
								resolve(newSolutionDir, i.toString()),
								code,
								"utf8",
							);
							i++;
			...
			complete: async (tester: FinalTesterResults) => {
				...
				Promise.all(
					[...originalPathToCopyPath.values()].map((copyPath) =>
						unlink(copyPath),
					),
