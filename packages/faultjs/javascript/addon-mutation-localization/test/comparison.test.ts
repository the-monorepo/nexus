...
describe.skip("sorting", () => {
		...
		for (let i = 0; i < orderedEvaluations.length - 1; i++) {
			...
			for (let j = i + 1; j < orderedEvaluations.length; j++) {
				...
				it(`${arrToString(orderedEvaluations[j])} greater than ${arrToString(
					...
					expect(compareMutationEvaluations(greater, lesser)).toBeGreaterThan(
						0,
					);
				});
			}
	...
	it(compareInstruction.name, () => {
		...
		const instruction1 = new Instruction<any>(
			Symbol("1"),
			dependenciesMap1,
			[],
			[],
		);
		const instruction2 = new Instruction<any>(
			Symbol("2"),
			dependenciesMap1,
			[],
			[],
		);
		const instruction3 = new Instruction<any>(
			Symbol("3"),
			dependenciesMap1,
			[],
			[],
		...
		const instruction4 = new Instruction<any>(
			Symbol("4"),
			dependenciesMap2,
			[],
			[],
		...
		nodeEvaluations
			.get(pathToPrimaryKey(filePath, path2))!
			.evaluations.push(mE3);
