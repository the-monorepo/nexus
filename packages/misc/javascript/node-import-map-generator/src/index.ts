...
const requireResolveForImportAliasPath = (path: string, requireCwd: string) => {
	...
	return resolveForImportAliasPath(
		require.resolve(path, { paths: [requireCwd] }),
	);
};
...
const addImportsFromDependencies = async (
	...
	while (stack.length > 0) {
		...
		for (const packageName of Object.keys(deps)) {
			...
			seen.add(key);
			const importInfo = await addImportsFromPackageName(
				packageName,
				currentCwd,
			);
...
const exportsToEntrypoints = (
	...
	const isMultiMappings = Object.keys(exportsObject).every((key) =>
		key.startsWith("."),
	);
...
const addImportsFromPackageName = async (packageName: string, cwd: string) => {
	try {
		...
		scopes[importMapCwd] = {
			...scopes[importMapCwd],
			...Object.fromEntries(
				entryPointsFromPackageJson(json, packageName, packageDir),
			),
		};
	...
	yargs(hideBin(process.argv)).command({
		...
		handler: async ({
			...
			try {
				...
				await addImportsFromDependencies(
