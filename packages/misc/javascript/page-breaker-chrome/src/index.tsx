...
const previewWords = computed(() => {
	...
	for (let i = 0; i < 5; i++) {
		const letterCount =
			Math.random() *
				(replaceWordState.character.max - replaceWordState.character.min) +
...
const MiscSettings = () => (
	<section>
		...
		<Field
			labelContent="Language preferences"
			contentClass={styles.flexStretch}
		>
			...
			>
				<div>
					<Button
						class={cx(styles.theme, styles.dark)}
						type="primary"
						index={0}
					>
