		...
		<tr>
			...
			{project.results.map((result) => {
				const colorWeight =
					range === 0
						? 0
						: ((result != null ? result : 0) - project.min) / range;
				const invertColorWeight = project.invertColors
					? 1 - colorWeight
					: colorWeight;
				const offset = 120;
				...
				return (
					<td style={`background-color: rgb(${red}, ${green}, ${blue});`}>
						<p>
							...
							{project.total
								? ` (${Math.round((result / project.total) * 100)}%)`
								: ""}
						</p>
					</td>
				);
...
const projectResultsToRankings = (
	...
	for (const projectResult of projectResults) {
		totalRanks += Math.max(
			...projectResult.results.map((result) =>
...
const Main = () => {
	...
	if (state.separateArtificial) {
		...
		examResults.push(
			projectResultsToExamResults(projectResults, "Exam scores"),
		);
		eInspectResults.push(
			projectResultsToRankings(projectResults, "Einspect@n scores"),
		);
	}
