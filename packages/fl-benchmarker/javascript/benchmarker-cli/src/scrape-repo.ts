...
const run = async () => {
	...
	const retrievePrs = async (after?): Promise<QueryPayload> => {
		...
		do {
			...
			if (data.data != null) {
				...
				log.warn(
					`Requesting PRs after ${after} responded with strange results...`,
				);
				console.log(data);
			}
	...
	do {
		...
		for (const pullRequest of data.data.repository.pullRequests.edges) {
			...
			const logSkipMessage = (reason: string) => {
				log.info(
					`Skipping ${chalk.cyanBright(pullRequest.node.title)}. ${reason}`,
				);
			};
			...
			const testFiles = pullRequest.node.files.nodes.filter(
				(file) =>
					!!file.path.match(
						/((\btest\b.*\.([tj]sx?|flow))|\binput\b|\bout\b)$/i,
					),
			);
			...
			spicyPullRequests.push({
				...
				oid: pullRequest.node.mergeCommit
					? pullRequest.node.mergeCommit.oid
					: null,
			});
		}
	...
	log.info(
		`Searched ${prCount} PRs - ${spicyPullRequests.length} have potential`,
	);
