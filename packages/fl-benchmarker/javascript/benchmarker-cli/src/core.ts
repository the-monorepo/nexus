...
export const run = async () => {
	...
	const runOnProject = async (projectDir: string) => {
		...
		if (selectedConfig === null) {
			log.warn(
				`Could not find an explicit config for '${chalk.cyanBright(projectDir)}'`,
			);
		}
		...
		const sbflAddons = sbflAlgorithms.map(
			({ scoringFn, name, console = false }) => {
