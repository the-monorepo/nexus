...
import { locationToKeyIncludingEnd, compareMutationEvaluations, DELETE_STATEMENT } from '@fault/addon-mutation-localization';
...
const main = async () => {
  ...
  for(const dir of projectDirs) {
    try {
      const mbflResults = convertFileFaultDataToFaults(require(resolve(dir, 'faults/mbfl/faults.json')));
      mutations.push(mbflResults.length);
      statements.push(await readFile(resolve(dir, 'faults/mbfl/mutations-attempted.txt'), 'utf8'))
      const index = mbflResults.findIndex(fault => fault.other.evaluation.length <= 0 || fault.other.evaluation[0].evaluation.overallPositiveEffect === 0);
      for(const algo of algos) {
        try {
          ...
          if (index === -1) {
            ...
            const otherResults = convertFileFaultDataToFaults(require(resolve(dir, `faults/${algo}/faults.json`)));
            const scores = new Map();
            for(const fault of otherResults) {
              scores.set(locationToKeyIncludingEnd(fault.sourcePath, fault.location), fault.score);
            }
            const fineScores = mbflResults.slice(0, index);
            let lowestFineScore = Math.min(...fineScores.map(fault => fault.score)) - 1;
            const mappedFineScores = fineScores.map(fault => ({...fault, score: lowestFineScore--}));
            const newResults = mappedFineScores.concat(mbflResults.slice(index).sort((a, b) => {
