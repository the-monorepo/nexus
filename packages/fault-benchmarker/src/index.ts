...
export const runOnProject = async (projectDir: string) => {
  ...
  const {
    setupFiles = [join(__dirname, 'babel')],
    testMatch = join(projectDir, '**/*.test.{js,jsx,ts,tsx}'),
  }: BenchmarkConfig = benchmarkConfigExists ? require(benchmarkConfigPath) : {};
  const expectedFaults = convertFileFaultDataToFaults(
    require(join(projectDir, 'expected-faults.json')),
  );
  ...
  const faultResultsPath = join(projectDir, 'fault-results.json');
  console.log(faultResultsPath);
