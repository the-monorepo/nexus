...
const callbackToStringCode = (taskCallback: TaskCallback): string => {
	...
	if (taskInfo !== undefined) {
		...
		while (i < flattened.length) {
			...
			if (
				...
				(childTaskInfo.value.length <= 1 ||
					childTaskInfo.type === flatteningType)
			) {
	...
	return chalk.cyanBright(
		taskCallback.name !== "" ? taskCallback.name : "anonymous",
	);
};
...
export const parallel = (...taskCallbacks: TaskCallback[]) => {
	const runInParallel: TaskCallback = async () => {
		await Promise.all(
			taskCallbacks.map((task) => task()).map(waitForTaskReturnValue),
		);
	};
