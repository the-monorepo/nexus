...
let requestRerender: undefined | Function<any> = undefined;
...
export const withHooks = <P, FCR>(Component: FC<P, FCR>): FC<P, any> => {
  const blueprint = createBlueprint(({ props }, container, before) => {
    ...
    renderOrReuseComponentResult(state.previousResult, componentResult, container, before);
  });
