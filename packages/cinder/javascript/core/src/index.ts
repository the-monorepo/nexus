...
export type MountFn<
	V,
	R extends StatelessCloneInfo<any> | RenderData<any, any>,
> = (v: V, container: Node, before: Node | null) => R;
...
export const createBlueprint: CreateBlueprintFunction = (
	mount,
	update?,
	unmount?,
) => ({
...
export const event = <E extends Element = any>(
	el: E,
	key: string,
): EventField => {
...
export const render = (value: unknown, container: Node) => {
	const renderResult = renderValue(
		value,
		trackedNodes.get(container),
		container,
		null,
	);
...
export const updateComponentResultsArray = <C, V, N extends Node>(
	...
	while (oldHead <= oldTail && newHead <= newTail) {
		if (results[oldHead] === null) {
			...
			isReusableRenderResult(newComponentResults[newHead], results[oldHead]!)
		) {
			...
			isReusableRenderResult(newComponentResults[newTail], results[oldTail]!)
		) {
			...
			isReusableRenderResult(newComponentResults[newTail], results[oldHead]!)
		) {
			...
			newRenderResults[newTail] = replaceOldResult(
				...
				results[oldHead]!,
				newTail + 1 < newLength
					? newRenderResults[newTail + 1].data.first
					: before,
			)!;
			newTail--;
			oldHead++;
		} else if (
			isReusableRenderResult(newComponentResults[newHead], results[oldTail]!)
		) {
...
const createDynamicElementBlueprint = <T>(
	createElementCallback: (v: T) => Element,
) => {
	const mount = (
		{
			value,
...
export const validateComponent = (component, properties) => {
	if (typeof component === "string") {
		return componentResult(dynamicTagNameBlueprint, {
			value: component,
			properties,
		});
	} else if (component?.prototype instanceof HTMLElement) {
		return componentResult(dynamicElementBlueprint, {
			value: component,
			properties,
		});
...
export const renderValue = (
	...
	return renderOrReuseComponentResult(
		componentResult,
		oldResult,
		container,
		before,
...
export const repeatBlueprint = createBlueprint(
	(
		initialInput: RepeatBlueprintInput<any, any, any>,
		initialContainer,
		before,
	) => {
		...
		for (const itemData of initialInput.values) {
			if (itemData != null) {
				const componentResult = componentResultFromValue(
					initialInput.mapFn(itemData, i),
				);
		...
		while (oldHead <= oldTail && newHead <= newTail) {
			if (results[oldHead] === null) {
				...
				newRenderResults[newTail] = movePart(
					...
					nextNewTail < newLength
						? newRenderResults[nextNewTail].data.first
						: before,
				);
				oldHead = nextOldHead;
				newTail--;
				...
				if (!newKeyToIndexMap.has(keys[oldHead])) {
					...
					if (oldIndex === undefined) {
						...
						newRenderResults[newHead] = movePart(
							...
							newComponentResults[newHead],
							container,
							nextOldIndex < oldLength
								? results[nextOldIndex]!.data.first
								: before,
							results[oldHead]!.data.first,
						);
