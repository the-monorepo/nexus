...
impl<E: ElementField> Blueprint<&str, Vec<&mut E>, ElementDomUpdater, ElementDomMounter>
    for ElementDomBlueprint
{
...
impl<E: ElementField> Mounter<Vec<&mut E>, ElementDomUpdater> for ElementDomMounter {
    fn mount(self, container: &mut web_sys::Node, before: &web_sys::Node) -> ElementDomUpdater {
        let document = web_sys::window().unwrap().document().unwrap();
        let cloned = document
            .import_node_with_deep(&self.template, true)
            .unwrap();
