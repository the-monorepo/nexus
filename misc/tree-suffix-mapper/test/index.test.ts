import { matchSuffix, createSuffixRoot, RetrievalObject, SuffixNode } from '../src/index';
...
const suffixNode = (value: number | undefined, ...valueSuffixPairs: [string, SuffixNode<string, number>][]): SuffixNode<string, number> => ({
  value,
  suffixes: new Map(valueSuffixPairs),
});

const keySuffixNodePair = (key: string, value: number | undefined, ...valueSuffixPairs: [string, SuffixNode<string, number>][]): [string, SuffixNode<string, number>] => ([
  key,
  suffixNode(value, ...valueSuffixPairs)
]);
...
describe('tree-suffix-mapper', () => {
  it('createSuffixMap', () => {
    const suffixMap = createSuffixRoot(
      testRetrievalObject,
      ...testTrees
    );
